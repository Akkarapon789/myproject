...
// 2. รับค่า promo_id ที่ส่งมาจาก JavaScript
$promo_id = isset($_POST['promo_id']) ? intval($_POST['promo_id']) : 0;
$discount = 0;
$final_total = $cart_total;

// 3. ถ้ามีการเลือกโปรโมชั่น (ID ไม่ใช่ 0)
if ($promo_id > 0) {
    ...
    if ($promo) {
        ...
        // 5. คำนวณยอดสุทธิ (ป้องกันไม่ให้ส่วนลดมากกว่าราคาสินค้า)
        $final_total = $cart_total - $discount;
        if ($final_total < 0) {
            $final_total = 0;
            $discount = $cart_total; // ถ้าส่วนลดเกิน ให้ส่วนลดเท่ากับราคาสินค้า
        }
    }
}

// 6. บันทึกผลลัพธ์ลง Session เพื่อใช้ในหน้า Checkout
$_SESSION['promo_id'] = $promo_id;
$_SESSION['promo_discount'] = $discount;
$_SESSION['final_total'] = $final_total;

// 7. ส่งข้อมูลที่คำนวณได้กลับไปให้ JavaScript ในรูปแบบ JSON
echo json_encode([
