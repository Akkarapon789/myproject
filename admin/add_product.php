...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
            ...
            foreach ($_FILES['images']['name'] as $i => $name) {
                if ($_FILES['images']['error'][$i] === UPLOAD_ERR_OK) {
                    ...
                    if (move_uploaded_file($_FILES["images"]["tmp_name"][$i], $targetFilePath)) {
                        ...
                        if ($main_image_url === null) $main_image_url = $image_path;
        ...
        // ⭐️⭐️⭐️ 2. แก้ไข SQL INSERT: ลบ `description` ออก ⭐️⭐️⭐️
        $stmt_product = $conn->prepare("INSERT INTO products (category_id, title, slug, price, stock, image_url) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt_product->bind_param("issdis", $category_id, $title, $slug, $price, $stock, $main_image_url);
        $stmt_product->execute();
        $last_product_id = $conn->insert_id;
        $stmt_product->close();

        // 3. บันทึกรูปภาพเพิ่มเติม (ส่วนนี้ถูกต้องแล้ว)
